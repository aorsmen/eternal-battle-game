...
import { formatValue } from "../DetailsPanelTabs/styled.components";
...
const TEST_OPEN_MAIN_CTX = {
  value: { ...TEST_MAIN_CONTEXT, getHeroById: () => TEST_HERO_DATA },
};

test("Should not render the details panel", () => {
  renderMainContext(<DetailsPanel />, {
...
test("Should render title and close button", () => {
  renderMainContext(<DetailsPanel />, {
    providerProps: TEST_OPEN_MAIN_CTX,
    withRouter: false,
  });

  const title = screen.getByText(TEST_HERO_DATA.name);
...
test("Should render hero avatar image", () => {
  renderMainContext(<DetailsPanel />, {
    providerProps: TEST_OPEN_MAIN_CTX,
    withRouter: false,
  });

  const image = screen.getByRole("img", { name: TEST_HERO_DATA.name });
